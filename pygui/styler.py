...
class Styler:
    """
    Modifier class to update positions and sizes of widgets when
    window is resized
    """
    ...
    def __init__(self, target):
        ...
        self.x_scale_percent = 1.0
        self.y_scale_percent = 1.0
    ...
    def update_size(self, pre_width, pre_height, new_width, new_height):
        ...
        if self.x_scale_type == STYLER_SCALETYPE_INCREMENT:
            self.target.width += delta_x * self.x_scale_percent

        if self.y_scale_type == STYLER_SCALETYPE_INCREMENT:
            self.target.height += delta_y * self.y_scale_percent
        

        #Scaling with fixed resolution
        if self.x_scale_type == STYLER_SCALETYPE_FIXED:
            twidth  = self.target.width * new_width / pre_width
            theight = self.target.height* new_width/ pre_width

            self.target.width = twidth
            self.target.height = theight
